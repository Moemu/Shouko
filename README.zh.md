...
**我们训练了一只果蝇来操控美少女的身体。以下是训练过程，以及如何在本地运行和训练。**

她的全名是 ショウジョウバエ。公开的雄性果蝇连接组数据是她的脑，美少女是 VRM 角色 [Yumi](research/avatar/YUMI.md)，实验跑在 MuJoCo 物理模拟里。

| G1 身体（pixiv 示例角色） | Yumi 身体 |
|---|---|
| ![完整连接组驱动的 G1 身体](assets/preview-g1.png) | ![完整连接组驱动的 Yumi 身体](assets/preview-yumi.png) |

## 概述
我们想验证一条工程链路：**把果蝇的真实神经接线接成一个控制网络，让它驱动一个身体走路。**

果蝇脑的**连接组**（connectome）记录的是“哪个神经元连到哪个”，不含大脑本身的功能。本项目把这份接线清单导入成神经网络，训练它输出 12 个关节动作，交给 MuJoCo 计算重力和接触，角色骨骼跟随同一份物理状态。
...
| **当前主线** | 完整连接组 | **Yumi 身体** | 已验收，本地可实时预览 |
...
提案来自 `research/experiments/` 的技术复盘。
...
两栏是同一个实时工作室：左侧面板渲染物理驱动的 VRM 角色与步态指标，右侧是 166,700 神经元连接组中 2,048 个采样胞体的活动率。
