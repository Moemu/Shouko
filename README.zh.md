...
Shouko 与前两者的不同首先在开放程度：训练方法、权重与记录全部公开，可以在本地复现。其次在进度上，这一点你需要提前知道：不同于前两者的完整四肢控制，Shouko 目前只控制下肢，上半身是一个固定质量块，实现了受控下的稳定平地行走（支持速度与朝向控制，没有引入目标点）。观感不会像前两个那么有趣，但完整四肢就是下一个阶段。我们对这两个项目做了固定版本的资料整理，见 [research/references/fly-embodiment/](research/references/fly-embodiment/README.md)。
...
浏览器端的云端预览服务正在筹备中：不久之后你就可以在自己的浏览器里看到她走路。欢迎社区复现（见[本地运行](#本地运行)与[复现训练过程](#复现训练过程)）与反馈建议；完整四肢、复杂地形、跑步、基于多巴胺的目标奖励，以及可能的小型语言模型等后续方向见[后续工作](#后续工作)一节。
