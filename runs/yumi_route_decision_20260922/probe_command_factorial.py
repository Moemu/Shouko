...
assert not (root/'command_factorial.json').exists()
report=dict(kind='paired_source_command_factorial',checkpoint_sha256=file_sha256(checkpoint),
            seeds=seeds,speed_grid=[0.35,0.5,0.65],turn_grid=[-0.2,0.0,0.2],seconds=12,command_observation_mean=policy.obs_mean[6:9].tolist(),command_observation_std=policy.obs_std[6:9].tolist(),command_scale=bodies[0].cfg['cmd_scale'],conditions=[],completed=False,
...
with ordered_inference():
    for speed,turn in itertools.product([0.35,0.5,0.65], [-0.2,0.0,0.2]):
        mode=f'speed_{speed}_turn_{turn}'
        speeds=[speed]*9
        ...
        report['conditions'].append(dict(mode=mode,forward_command=speed,turn_command=turn,tests=tests,wall_seconds=time.perf_counter()-started))
        (root/'command_factorial.json').write_text(json.dumps(report,indent=2,allow_nan=False))
