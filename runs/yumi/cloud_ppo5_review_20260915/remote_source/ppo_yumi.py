...
def train(args):
    ...
    if args.resume_state and ppo_state_path.exists():
        state = torch.load(ppo_state_path, weights_only=True, map_location=device)
        value.load_state_dict(state['value'])
        log_std.data.copy_(state['log_std'])
        print(json.dumps(dict(resumed_ppo_state=True)), flush=True)
    ...
    env.command[:, 2] = torch.linspace(-0.2, 0.2, env.worlds, device=device)
    ...
    while time.perf_counter()-started < args.max_seconds:
        ...
        # ---- update ----
        policy.train()
        pi_loss_total = v_loss_total = 0.0
        count = 0
        for epoch in range(args.epochs):
            ...
            for start in range(0, len(obs_b), args.minibatch):
                ...
                v_loss.backward()
                value_opt.step()
                pi_loss_total += float(pi_loss); v_loss_total += float(v_loss); count += 1
        iteration += 1
